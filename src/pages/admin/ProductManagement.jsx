...
import { FiPlus, FiEdit2, FiTrash2, FiUploadCloud, FiCheck, FiPercent } from 'react-icons/fi';
...
export default function ProductManagement() {
  ...
  const handleImageUpload = async (e) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        console.warn('Firebase Storage upload fallback to Base64 device image:', err.message);
  ...
  const handleDiscountChange = (val) => {
    ...
    setForm((prev) => ({ ...prev, discountPercent: val, mrp: mrp > 0 ? mrp : prev.mrp, price: newPrice }));
  };
  ...
  const applyPresetDiscount = (pct) => {
    ...
    setForm((prev) => ({ ...prev, mrp: mrp, discountPercent: pct, price: newPrice }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      images: form.images.filter(Boolean).length ? form.images.filter(Boolean) : ['/images/rb_protein_tub.jpg'],
      status: 'approved',
    };
  ...
  const handleEdit = (product) => {
    ...
    const disc = mrp > 0 && mrp > price ? Math.round(((mrp - price) / mrp) * 100) : '';
    ...
    setForm({
      ...
      images: product.images?.length ? product.images : ['/images/rb_protein_tub.jpg'],
    });
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Add new products, change images, update text descriptions, set discount %, and calculate selling price.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_360px] xl:grid-cols-[1fr_400px] gap-6 items-start">
        ...
        <div className="card overflow-hidden">
          <div className="p-4 border-b border-gray-200 dark:border-brand-border flex items-center justify-between">
            <h2 className="font-bold text-base">Store Catalog ({products.length} Products)</h2>
          </div>
          {products.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[560px] text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-brand-border">
                  {products.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-brand-surface/50 transition-colors">
                      <td className="p-3.5 pl-4 flex items-center gap-3 min-w-[180px]">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-xs sm:text-sm line-clamp-1">{p.name}</p>
                          <p className="text-[11px] text-gray-500 line-through">₹{p.mrp || p.price}</p>
                        </div>
                      </td>
                      ...
                      <td className="p-3.5 whitespace-nowrap font-bold text-sm">₹{p.price}</td>
                      <td className="p-3.5 whitespace-nowrap">
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full whitespace-nowrap ${p.stock > 0 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                          {p.stock} in stock
                        </span>
                      </td>
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-4 shadow-card sticky top-20 border border-gray-200 dark:border-brand-border">
                ...
                <div className="flex gap-1">
                  {PRESET_IMAGES.map((preset) => (
                    <button
                      key={preset.url}
                      type="button"
                      onClick={() => setForm((prev) => ({ ...prev, images: [preset.url] }))}
          ...
          {/* Product Name */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">Product Title / Name</label>
            <input
              required
              type="text"
          ...
          <div className="space-y-3 p-3.5 rounded-2xl bg-gray-50 dark:bg-brand-surface/70 border border-gray-200 dark:border-brand-border">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                <label className="block text-[11px] font-semibold text-gray-400 mb-1">Original MRP (₹)</label>
                <input
                  type="number"
                  placeholder="3499"
                  value={form.mrp}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-yellow-500 mb-1">Discount (%)</label>
                <input
                  type="number"
                  placeholder="20"
              ...
              <div>
                <label className="block text-[11px] font-semibold text-brand-green-500 mb-1">Selling Price (₹)</label>
                <input
                  required
                  type="number"
                  placeholder="2799"
            ...
            <div className="flex items-center gap-1 pt-1 overflow-x-auto">
              <span className="text-[10px] text-gray-400 font-semibold shrink-0">Quick Discount:</span>
            ...
            {form.price && (
              <div className="p-3 rounded-xl bg-brand-green-500/10 border border-brand-green-500/30 space-y-1">
                <div className="flex items-center justify-between text-xs font-bold">
                  <span className="text-gray-300">Calculated Selling Price:</span>
                  <span className="text-base text-brand-green-500 font-extrabold">₹{Number(form.price).toLocaleString('en-IN')}</span>
                </div>
                {Number(form.mrp) > Number(form.price) && (
                  <div className="text-[11px] text-gray-400 flex items-center justify-between">
                    <span>Original MRP: <span className="line-through">₹{Number(form.mrp).toLocaleString('en-IN')}</span></span>
                    <span className="text-brand-green-400 font-bold">
                      Save ₹{(Number(form.mrp) - Number(form.price)).toLocaleString('en-IN')} ({Math.round(((Number(form.mrp) - Number(form.price)) / Number(form.mrp)) * 100)}% OFF)
                    </span>
                  </div>
                )}
              </div>
          ...
          {/* Description Text */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">Product Description Text</label>
            <textarea
              rows={3}
